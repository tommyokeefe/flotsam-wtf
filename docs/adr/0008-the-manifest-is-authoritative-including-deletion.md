...
**Amended by #43:** CI now does this. The `Publish and deploy` workflow publishes on every push to `main`, from the run for the tip of `main` only, and deploys afterwards whether or not publishing succeeded. ADR 0009 covers the deploy side.
